...
此方法将处理参数以确定orderers。处理程序将使用提供的orderer或使用分配给通道的orderer。处理程序应执行故障转移，并使用所有可用的orderer程序发送已认可的交易。

- 参数

| 名称   | 类型                    | 描述                             |
| ------ | ----------------------- | -------------------------------- |
| params | CommitHandlerParameters | 一个EndorsementHandlerParameters |
